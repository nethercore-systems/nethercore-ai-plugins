...
def main(spec_path, output_path):
    ...
    export_character(armature, merged, output_path)

    print(f"Character exported to: {output_path}")
    print(f"Triangle count: {len(merged.data.polygons)}")
...
Run: blender --background --python generate_character.py
"""
...
if __name__ == "__main__":
    clear_scene()
    armature, merged = generate_character(SPEC)
    export_character(armature, merged, "character.glb")
```
...
| Function | Purpose |
|----------|---------|
| `create_armature(skeleton)` | Build armature from bone definitions |
| `get_bone_transform(armature, bone_name)` | Get matrix to orient mesh along bone |
| `build_body_part(spec, armature)` | Create single part via extrude+scale |
| `build_instances(spec, armature, name)` | Create multiple instances (hair, etc.) |
| `validate_spec(spec)` | Check spec for errors before generation |
| `estimate_tris(spec)` | Calculate expected triangle count |
| `check_seam_compatibility(parts, warnings)` | Verify connected parts have matching vertices |
| `apply_uvs(obj, spec)` | Apply UV mapping (smart_project or region_based) |
| `merge_and_skin(parts, armature, spec)` | Join parts, weld seams, apply weights |
| `generate_character(spec)` | Main entry - returns (armature, mesh) |
| `export_character(armature, mesh, path)` | Export to GLB |
