...
|----------|----------|---------------|
| **Python + numpy/scipy** | Asset generation (Recommended) | .py files, outputs PNG/WAV |
| Blender bpy | 3D mesh generation | .py files, outputs .glb/.gltf |
| Rust + proc-gen | Game code only (NOT asset generation) | .rs files (WASM) |
...
### Recommended Structure (Python Asset Generation)

```
generator/              # Python asset generation scripts
├── generate.py         # Main entry point
├── textures/
│   ├── __init__.py
│   └── materials.py    # Texture generation with numpy/PIL
├── meshes/
│   └── primitives.py   # Mesh generation with bpy (Blender)
├── audio/
│   └── synthesis.py    # Audio synthesis with numpy/scipy
...
├── textures/           # PNG files generated by Python
├── meshes/             # GLB files generated by Blender bpy
└── audio/              # WAV files generated by Python
...
def generate_all_assets(spec: AssetSpec):
    # Everything in one function

# ✅ GOOD: Composed from small functions
def generate_all_assets(spec: AssetSpec):
    mesh = generate_mesh(spec.mesh)
    textures = generate_textures(spec.textures)
    materials = generate_materials(spec.materials)
...
add_noise_displacement(obj, seed=seed, strength=0.05)

# 2. Bake ambient occlusion (THE key technique)
bake_vertex_ao(obj)

# 3. Bake curvature for edge wear
bake_vertex_curvature(obj)

# 4. Bake key light into vertex colors
bake_directional_light(obj,
...
snap_uvs_to_pixel_grid(obj, resolution=256, half_pixel_offset=True)

# Export to GLB
bpy.ops.export_scene.gltf(filepath='barrel.glb', export_format='GLB')
```
...
# Create layered texture with numpy + PIL
def generate_texture(width: int = 256, height: int = 256, colors: dict = None, seed: int = 0):
    # Base color
    texture = np.full((height, width, 4), colors['primary'], dtype=np.uint8)

    # Noise layer (Perlin overlay)
    noise = FastNoiseLite(seed)
    noise.noise_type = NoiseType.NoiseType_Perlin
    noise_layer = generate_noise_layer(width, height, noise, colors['dark'], colors['light'])
    texture = blend_overlay(texture, noise_layer, alpha=0.3)

    # Scratches
    scratches = generate_scratches(width, height, density=0.2, depth=0.4, seed=seed)
    texture = blend_multiply(texture, scratches)

    # Cracks
    cracks = generate_cracks(width, height, density=0.1, depth=0.3, seed=seed+1)
    texture = blend_multiply(texture, cracks)

    # Weathering pass
    weathering = generate_weathering(width, height, damage=0.5, dark_color=colors['dark'], seed=seed+2)
    texture = blend_overlay(texture, weathering, alpha=0.6)

    # Final pass (contrast + brightness)
    texture = apply_final_pass(texture, sharpen=0.1, darken=colors['dark'], contrast=1.05, seed=seed+3)

    return Image.fromarray(texture)

# Validate quality
report = assess_texture_quality(texture)
if not report.is_good():
    print(f"Quality issues: {report.issues}")

# Save as PNG
Image.fromarray(texture).save('barrel_albedo.png')
```

### Using Style Recipes

```python
from sadl import GenerationRecipe

# Create recipe from description
recipe = GenerationRecipe.from_description("weathered medieval barrel", seed)

# Generate with recipe
texture = recipe.generate_texture(seed)  # Returns PIL.Image
mesh = recipe.generate_mesh(seed)  # Returns bpy mesh object

# Export
texture.save('barrel_albedo.png')
recipe.export_mesh(mesh, 'barrel.glb')
```
